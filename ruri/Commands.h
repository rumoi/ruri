	...
	void RecalcSingleUserPP(const u32 ID, _SQLCon &SQL){//This is relatively expensive. Obviously if there was an entire pp recalc a per beatmap solution would be much better (and will be made).
	...
	const std::string ProcessCommand(_User* u,const std::string_view Command, DWORD &PrivateRes){
		...
		const u32 CommandHash = WSTI<u32>(Split[0]);
		//User Commands
		{
			switch (CommandHash) {
		...
		//Admin Commands
		{
			...
			case WSTI("!restrictid"):{
				...
				UpdateQue.emplace_back(TargetID, u->UserID,
					Restrict ? _UserUpdate::Restrict : _UserUpdate::UnRestrict, (size_t)new std::string(CombineAllNextSplit(2, Split)));
