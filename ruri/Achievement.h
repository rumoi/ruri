...
struct Achievement{
	...
	std::array<u32, GM_MAX + 1> General;
	u64 Hushush;

	Achievement() : General{}, Hushush(0){}
};
...
const char* const Ach_GeneralOsu[] = {
...
const char* const Ach_GeneralTaiko[] = {
...
const char* const Ach_GeneralCTB[] = {
...
const char* const Ach_GeneralMania[] = {
...
const char* const* const Ach_GeneralNames[] = { Ach_GeneralOsu, Ach_GeneralTaiko, Ach_GeneralCTB, Ach_GeneralMania};

void Ach_General(const u32 Diff, const u32 GameMode, std::string& res){
	for (size_t i = Diff ? 0 : -1; i < 20; i++)
		if (Diff & (1 << i))
			i < 4 ? res.append(nAch(ComboSteps[i] + " Combo", ComboSteps[i] + " big ones! You're moving up in the world!", "osu-combo-" + ComboSteps[i])) :
				res.append(Ach_GeneralNames[GameMode][i - 4]);
}

#undef nAch

void CalculateAchievement(Achievement &New, const Achievement Old, const size_t GameMode, std::string &res){

	u32 Diff = 0;
	New.General[GameMode] = Old.General[GameMode] | (Diff = (New.General[GameMode] ^ Old.General[GameMode]) & New.General[GameMode]);

	Ach_General(Diff, GameMode & 3, res);
